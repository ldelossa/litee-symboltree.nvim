...
function M._setup_buffer(name, buf, tab)
    ...
    if config.auto_highlight then
        vim.api.nvim_create_autocmd({"BufWinLeave", "WinLeave"}, {
            buffer = buf,
            callback = function () require('litee.symboltree.autocmds').auto_highlight(false) end
        })
        vim.api.nvim_create_autocmd({"CursorHold"}, {
            buffer = buf,
            callback = function () require('litee.symboltree.autocmds').auto_highlight(true) end
        })
